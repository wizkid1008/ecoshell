import {cleanString, generateToken, json, requireEnv, supabaseFetch} from '../../_shared/supabase.js';
...
export async function onRequestGet({request, env}) {
  ...
  try {
    ...
    const matches = await supabaseFetch(
      env,
      `client_login_tokens?token=eq.${encodeURIComponent(token)}&kind=eq.magic&used_at=is.null&expires_at=gt.${encodeURIComponent(nowIso)}&select=id,email`
    );

    const magicToken = matches[0];
    if (!magicToken) return json({error: 'This login link is invalid or has expired.'}, {status: 401});

    await supabaseFetch(env, `client_login_tokens?id=eq.${magicToken.id}`, {
      method: 'PATCH',
